...
const ActionsBar = () => {
  ...
  const filteredActions = actionsList.filter((action) => action.toLowerCase().includes(search.toLowerCase()));
  return (
      ...
      {openAction && (
        <div className="absolute left-0 right-0 top-full mt-1 z-9999999 bg-surface border border-text-muted/70 rounded-lg shadow-lg p-4 flex flex-col gap-2 text-xs">
          <div className="flex justify-between items-center mb-2">
            <span className="font-semibold text-text-main ">{openAction}</span>
          </div>
          <div className="text-text-main ">{renderContent(openAction)}</div>
        </div>
      )}
    </div>
  );
